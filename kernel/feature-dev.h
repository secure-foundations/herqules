...
#define FME_FEATURE_ID_PAC_HSSI_ETH 0xa
#define FME_FEATURE_ID_S10_SDM_MB 0xb
#define FME_FEATURE_ID_AVMMI_BMC 0xc
#define FME_FEATURE_ID_NIOS_SPI 0xd
#define FME_FEATURE_ID_MAX10_SPI 0xe
#define FME_FEATURE_ID_MAC_ROM_I2C 0xf
#define FME_FEATURE_ID_ETH_GROUP 0x10
#define FME_FEATURE_ID_TCM 0x11
...
#define FME_HSSI_ETH_REVISION 1
#define FME_GLOBAL_DPERF_REVISION 0
#define FME_QSPI_REVISION 0
#define FME_EMIF_MGMT_REVISION 0

#define PORT_HEADER_REVISION 1
...
#define PORT_IOPLL_REVISION 0

#define FME_FEATURE_HSSI_ETH_SIZE 0x18
#define FME_FEATURE_AVMMI_BMC_SIZE 0x18
...
struct feature_fme_header {
    ...
    struct feature_fiu_header fiu_header;
    u64 reserved;
    u64 scratchpad;
...
#define FME_ERROR0_MASK_DEFAULT 0x40ULL /* pcode workaround */
    union {
        u64 csr;
        struct {
...
#define FME_PCIE0_ERROR_MASK 0xFFULL
    union {
        u64 csr;
        struct {
...
#define FME_PCIE1_ERROR_MASK 0xFFULL
    union {
        u64 csr;
        struct {
            u8 formattype_err : 1;   /* TLP format/type error */
...
#define FME_RAS_ERROR_INJ_MASK 0x7ULL
    union {
        u64 csr;
        struct {
...
struct feature_fme_pr_protect {
    union {
        u64 csr;
        struct {
            u32 max10_doorbell;   /* Mirror of max10 doorbell */
            u8 hw_pr_control : 1; /* HW has PR Control */
...
struct feature_fme_pr {
    ...
    struct feature_fme_pr_protect fme_pr_protect;

    /* MSIX filed to be Added */
};

/* FME EMIF Status */
struct feature_fme_emif_status {
    union {
        u64 csr;
        struct {
            u8 complete : 8;    /* Initialization Complete */
            u8 cal_failure : 8; /* Calibration Failure */
            u8 active : 8;      /* Clear State Machine Active */
            u64 rsvd : 40;      /* Reserved */
        };
    };
};
...
/* FME EMIF Control */
#define EMIF_CLEAR_MASK 0xff
struct feature_fme_emif_control {
    union {
        u64 csr;
        struct {
            u8 clear : 8;  /* EMIF Clear State Machine Enable*/
...
/* FME EMIF FEATURE */
struct feature_fme_emif {
    struct feature_header header;
    struct feature_fme_emif_status status;
    struct feature_fme_emif_control control;
};

struct feature_fme_s10_sdm_mb {
    u32 ctrl;
    u32 addr;
    u32 writedata;
    u32 readdata;
...
#define PORT_ERR_MASK 0xf0fff0703ff061f
struct feature_port_err_key {
    union {
        ...
        struct {
            ...
            /* Byte Length cannot be zero, minimum byte length is 1 */
            u8 byte_len_err : 1;
            /* The sum of byte start index and byte length not exceed 64*/
            u8 allowbyte_len_err : 1;
            /* Cache length cannot be non-zero during byte enable mode*/
            u8 cachelen_mode_err : 1;
            /* Byte len and Byte start fields are non zero*/
            u8 byte_mode_err : 1;
...
struct feature_platform_data {
    ...
    struct platform_device *(*fpga_for_each_port)(
...
feature_dev_use_excl_begin(struct feature_platform_data *pdata) {
    ...
    if (pdata->open_count || pdata->unplug) {
...
static inline void fpga_pdata_set_private(struct feature_platform_data *pdata,
                                          void *private) {
    pdata->private = private;
}

static inline void *
fpga_pdata_get_private(struct feature_platform_data *pdata) {
 ...
 * @n: id of this feature driver
 * @p: ops of this feature driver
 *
 * This macro is used to create a struct feature_driver that matches a
 * specific feature.
 */
#define FEATURE_DRV(n, p) .id = (n), .ops = (p)
...
void feature_platform_data_add(struct feature_platform_data *pdata, int index,
                               const char *name, int resource_index,
                               void __iomem *ioaddr,
                               struct feature_irq_ctx *ctx,
                               unsigned int ctx_num);
int feature_platform_data_size(const int num);
struct feature_platform_data *
...
fpga_inode_to_feature_dev(struct inode *inode) {
...
static inline struct feature *get_feature_by_id(struct device *dev, u64 id) {
    ...
    fpga_dev_for_each_feature(pdata,
                              feature) if (feature->id == id) return feature;

    return NULL;
}

static inline void *get_feature_priv_by_id(struct device *dev, u64 id) {
    struct feature *feature = get_feature_by_id(dev, id);

    if (feature && feature->priv)
        return feature->priv;
...
static inline void __iomem *get_feature_ioaddr_by_id(struct device *dev,
                                                     u64 id) {
    struct feature *feature = get_feature_by_id(dev, id);
...
static inline bool is_feature_present(struct device *dev, u64 id) {
    return get_feature_by_id(dev, id) ? true : false;
}
...
enum fpga_id_type {
    ...
    PORT_ID,
    /*
     * feature id allocation and mapping, this id is used in feature
     * driver, as sometimes feature driver needs one unique id to
     * distinguish different features with same private feature id.
     */
    FEATURE_ID,
    FPGA_ID_MAX,
};

int alloc_fpga_id(enum fpga_id_type type, struct device *dev);
void free_fpga_id(enum fpga_id_type type, int id);
