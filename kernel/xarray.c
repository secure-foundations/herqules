 ...
 */
int __xa_alloc(struct xarray *xa, u32 *id, void *entry, struct xa_limit limit,
               gfp_t gfp) {
    XA_STATE(xas, xa, 0);
...
void xa_dump_node(const struct xa_node *node) {
    ...
    pr_cont("node %px %s %d parent %px shift %d count %d values %d "
            ...
            node, node->parent ? "offset" : "max", node->offset, node->parent,
            node->shift, node->count, node->nr_values, node->array,
            node->private_list.prev, node->private_list.next);
    ...
    if (xa_is_node(entry)) {
        if (shift == 0) {
            ...
            for (i = 0; i < XA_CHUNK_SIZE; i++)
                xa_dump_entry(node->slots[i], index + (i << node->shift),
                              node->shift);
        }
...
void xa_dump(const struct xarray *xa) {
    ...
    pr_info("xarray: %px head %px flags %x marks %d %d %d\n", xa, entry,
            xa->xa_flags, xa_marked(xa, XA_MARK_0), xa_marked(xa, XA_MARK_1),
