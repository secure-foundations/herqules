...
#define XA_LIMIT(_min, _max)                                                   \
    (struct xa_limit) { .min = _min, .max = _max }
...
#define XARRAY_INIT(name, flags)                                               \
    {                                                                          \
        .xa_lock = __SPIN_LOCK_UNLOCKED(name.xa_lock), .xa_flags = flags,      \
        .xa_head = NULL,                                                       \
    }
...
#define xa_for_each_range(xa, index, entry, start, last)                       \
    for (index = start, entry = xa_find(xa, &index, last, XA_PRESENT); entry;  \
...
#define xa_for_each_marked(xa, index, entry, filter)                           \
    for (index = 0, entry = xa_find(xa, &index, ULONG_MAX, filter); entry;     \
...
void *__xa_cmpxchg(struct xarray *, unsigned long index, void *old, void *entry,
                   gfp_t);
int __must_check __xa_insert(struct xarray *, unsigned long index, void *entry,
                             gfp_t);
 ...
 */
static inline int __must_check xa_insert(struct xarray *xa, unsigned long index,
                                         void *entry, gfp_t gfp) {
    int err;
 ...
 */
static inline __must_check int xa_alloc(struct xarray *xa, u32 *id, void *entry,
 ...
 */
static inline __must_check int xa_reserve(struct xarray *xa,
 ...
 * Return: 0 if the reservation succeeded or -ENOMEM if it failed.
 */
static inline __must_check int xa_reserve_bh(struct xarray *xa,
                                             unsigned long index, gfp_t gfp) {
 ...
 * Return: 0 if the reservation succeeded or -ENOMEM if it failed.
 */
static inline __must_check int xa_reserve_irq(struct xarray *xa,
                                              unsigned long index, gfp_t gfp) {
...
#define __XA_STATE(array, index, shift, sibs)                                  \
    {                                                                          \
        .xa = array, .xa_index = index, .xa_shift = shift, .xa_sibs = sibs,    \
        .xa_offset = 0, .xa_pad = 0, .xa_node = XAS_RESTART, .xa_alloc = NULL, \
        .xa_update = NULL                                                      \
    }
...
#define XA_STATE_ORDER(name, array, index, order)                              \
    struct xa_state name = __XA_STATE(array, (index >> order) << order,        \
...
static inline bool xas_top(struct xa_node *node) { return node <= XAS_RESTART; }
