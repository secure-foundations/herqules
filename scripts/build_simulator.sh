...
mkdir build_hq-sim
cd build_hq-sim
cmake -DTEST_SUITE_SUBDIRS=External -DCMAKE_C_COMPILER=$CLANG_CFI -DCMAKE_CXX_COMPILER=$CLANGXX_CFI -C../cmake/caches/O3.cmake -DTEST_SUITE_SPEC2006_ROOT=$SPEC2006 -DTEST_SUITE_SPEC2017_ROOT=$SPEC2017 -DCMAKE_CXX_FLAGS="$CFLAGS_CFI" -DCMAKE_C_FLAGS="$CFLAGS_CFI" -DCMAKE_EXE_LINKER_FLAGS="$LDFLAGS_CFI" -DTEST_SUITE_USE_PERF=ON -DTEST_SUITE_RUN_TYPE=train -DTEST_SUITE_COLLECT_CODE_SIZE=OFF -GNinja ..
ninja
cd ../../..
