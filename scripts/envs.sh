...
export LDFLAGS_NONE="-Wl,-z,now -Wl,-z,relro --target=x86_64-pc-linux-musl --sysroot=${CROSS_NONE} --gcc-toolchain=${CROSS_NONE} -flto -fuse-ld=gold -Wl,-I,/lib/ld-musl-none-x86_64.so.1"
...
export LDFLAGS_CPI="-pie -Wl,-z,now -Wl,-z,relro -target x86_64-pc-linux-musl -gcc-toolchain ${CROSS_NONE} --sysroot=${CROSS_NONE} -flto -fcpi -fsafe-stack -Wl,-I,/lib/ld-musl-none-x86_64.so.1"
...
export LDFLAGS_CCFI="-pie -Wl,-rpath,${CCFI}/Release+Asserts/lib -L${CCFI}/Release+Asserts/lib -lccfi_rt -Wl,-z,now -Wl,-z,relro -target x86_64-pc-linux-musl -gcc-toolchain ${CROSS_CCFI} --sysroot=${CROSS_CCFI} -flto -Wl,-I,/lib/ld-musl-ccfi-x86_64.so.1"
